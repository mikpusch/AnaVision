...
void StepperDopr853<D>::step(const Doub htry,D &derivs) {
        ...
        for (;;) {
				...
				if (fabs(h) <= fabs(x)*EPS){
                        NR_throw("stepsize underflow in StepperDopr853");
				}
				//ShowFloat(DormandPrince::MAXSTEP, "StepperDopr853::MAXSTEP");
				if (istep>DormandPrince::MAXSTEP){
					return;
				}
        }
...
Doub StepperDopr853<D>::error(const Doub h) {
        ...
        for (Int i=0;i<n;i++) {
				...
				Doub temp1=yerr[i]/sk;
                err2 += temp1*temp1; //SQR(yerr[i]/sk);
				Doub temp2 = yerr2[i]/sk;
                err += temp2*temp2; //SQR(yerr2[i]/sk);
        }
        deno=err+0.01*err2;
        if (deno <= 0.0)
                deno=1.0;
