...
void Markov::CalcHistogram(UINT StateCode, double * rates, double * coeffs, bool * IsRealEV){
	int nsubstates = 0;
...
//						QEE[I][J] = R[i][j];
    ...
    bool AllEVReal = CalcEigenValues(nsubstates, QEE, rates);
...
*/
	ComplexEV ev;
	ev.CalcEigenValuesAndVectors(nsubstates, nm, tau, e, ie, EVIsReal);
...
//	DMulMat(IW, W, QEE, nsubstates, nsubstates, nsubstates);
...
void Markov::CalcFullOmega(double *** omega){
