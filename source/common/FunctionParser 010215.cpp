...
double exp10(double arg1){
	return pow(10.0, arg1);
}

double logten(double arg1){
	return log10(arg1);
}
...
bool Function::NeedParenthesisForMultiply(){
  switch (Type) {
	...
	case 4 : if (binaryfunction==times){
					return false;
			 }
			 else{
				 return true;
...
Function * FunctionParser::ParseAuxNewBad(CString is, BOOL & ErrorOccured){
	//Alert(CString("Enter new parse with: ")+is);
	ErrorOccured=FALSE;
	int l= is.GetLength();
	if (l==0) return NULL;
...
//			return exprs[0];
		...
		Function * nf = NewFunc();
		nf->Type=4;
		nf->firstarg = new Function;
		exprs[mini]->Copy(*nf->firstarg);
//		nf->firstarg=exprs[mini];
		nf->secondarg = new Function;
