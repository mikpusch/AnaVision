/*
This file implements the Praxis algorithm
...
void Praxis::InitPraxis( int n,    // number of vars
					...
					int * MessageStuff){  // if you want to send messages
 ...
 // ShowInt(int(f), "F");
  ...
  DIMENS = n;
  STUFF = stuff;
  CURRENTBESTPARAMS = CurrentBestParams;
  MSGFUNC = msgfunc;
  T0 = Tolerance;
  if (!f){
	  return;
...
void Praxis::Minimize(double Tolerance,  double MaxStepSize, double * xarray, double & Fmin){
	if (!Initialized){
		return;
	}
	int prin=0;
	double result = praxis_(&(Tolerance),
                    &(MACHEP),
                    &( MaxStepSize),
                    &(DIMENS),
                    &(prin),
...
//                    FPraxis *f, // Functor for function under investigation
                    COSTFUNC, // Functor for function under investigation
                    &(Fmin) );
	Fmin = result;
...
void Praxis::CopyBestParams(double result, double * xarray){
	BestResult = result;
	if (CURRENTBESTPARAMS){
		for (int i=0; i<DIMENS; i++){
			CURRENTBESTPARAMS[i] = xarray[i];
		}
	}
}

double Praxis::EvalFunc(double * xarray){
	double result;
//	ShowInt(int(COSTFUNC), "COSTFUNC");

//	ShowFloat(xarray[0], "x[0]");
	(*COSTFUNC)(xarray, &result, STUFF);
//	ShowFloat(result, "Result");
	...
	if (result<BestResult){
		CopyBestParams(result, xarray);
		if (MSGFUNC){
			(*MSGFUNC)( MESSAGESTUFF);
		}
	}
	if (STATUSINT){
		if  ( (*STATUSINT) == 2){
...
/* Subroutine */ int Praxis::minfit_(C_INT *m, C_INT *n, C_FLOAT64 *machep,
...
int Praxis::min_(C_INT *n, C_INT *j, C_INT *nits, C_FLOAT64 * d2,
//								   C_FLOAT64 *x1, C_FLOAT64 *f1, bool *fk, FPraxis *f, C_FLOAT64 *
								   C_FLOAT64 *x1, C_FLOAT64 *f1, bool *fk, praxis_costfunction_type f, C_FLOAT64 *
...
//  static double small;
  ...
  t2 = my_dmax(t2, smalll);
  /* Computing MIN */
  d__1 = t2, d__2 = *h__ * .01;
  t2 = my_dmin(d__1, d__2);
...
L14:
  ...
  if ((d__1 = x2 * (x2 - *x1), fabs(d__1)) <= smalll)
    {
      goto L15;
    }
...
C_FLOAT64 Praxis::flin_(C_INT *n, C_INT *j, C_FLOAT64 *l, praxis_costfunction_type f, C_FLOAT64 *x,
                         C_INT *nf)
{
  /* System generated locals */
  C_INT i__1;
...
/* Subroutine */ int Praxis::quad_(C_INT *n, praxis_costfunction_type f, C_FLOAT64 *x, C_FLOAT64 *t,
...
C_FLOAT64 Praxis::praxis_(C_FLOAT64 *t0, C_FLOAT64 *machep, C_FLOAT64 *h0,
                           C_INT *n, C_INT *prin, C_FLOAT64 *x, praxis_costfunction_type f, C_FLOAT64 *fmin)
{
  /* System generated locals */
  ...
  static C_INT i__, j, k;
  static C_FLOAT64 s, t, y[100], large, z__[100], smalll, value, f1;
  static C_INT k2;
...
//  C_FLOAT64 lastValue = std::numeric_limits<C_FLOAT64>::infinity();
  C_FLOAT64 lastValue = 1e300; //std::numeric_limits<C_FLOAT64>::infinity();
  ...
  /* Function Body */
  smalll = *machep * *machep;
  vsmall = smalll * smalll;
  large = 1. / smalll;
  vlarge = 1. / vsmall;
  m2 = sqrt(*machep);
  m4 = sqrt(m2);
...
//          s = (global_1.ldt * .1 + t2 * pow(10.0, (C_FLOAT64)kt)) * (mpRandom->getRandomCC() - 0.5);
          s = (global_1.ldt * .1 + t2 * pow(10.0, (C_FLOAT64)kt)) * (Rand() - 0.5);
          z__[i__ - 1] = s;
          i__3 = *n;
...
int Praxis::print_(C_INT *n, C_FLOAT64 *x, C_INT *prin, C_FLOAT64 *fmin){
	return 0;
}
